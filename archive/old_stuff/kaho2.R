...
for(k in 1:nrow(Virus_info)){
  ...
  number_of_nucs <-length(as.list(al2)[[12]])
  #end<-360
...
al<-read.fasta("RhinovirusB.fasta_pruned.mu (modified).fasta", as.string=T)
pos<-c()
stop.seq.no<-c()
...
if (length(stop.seq.no)!=0 ){
  al<-al[stop.seq.no]
  write.fasta(al,names(al),"RhinovirusB.filtered.fasta" )

  if (length(poss)!=0){
    al.filtered<-read.dna("RhinovirusB.filtered.fasta", format = "fasta")
    al.trimmed<-al.filtered[,-c(poss*3-2, poss*3-1,poss*3)]
    write.FASTA(al.trimmed, file="humanparainfluenzavirus3_HN.trimmed.fasta" )}
  else  write.FASTA(al, file="humanparainfluenzavirus3_HN.trimmed.fasta" )
}

if (length(stop.seq.no)==0){
  if (length(poss)!=0){
    al.filtered<-read.dna("RhinovirusB.fasta", format = "fasta")
    al.trimmed<-al.filtered[,-c(poss*3-2, poss*3-1,poss*3)]
    write.FASTA(al.trimmed, file="RhinovirusB.trimmed.fasta" )}
  else file.copy("RhinovirusB.fasta", "RhinovirusB.fasta" )
}
