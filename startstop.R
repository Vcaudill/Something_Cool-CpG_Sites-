meanFreq <- function(fasta_file){
 ...
 # fasta_file <- "EnterovirusA_VP1.fasta_pruned.mu.trim05"
  virus_basic <- read.fasta(fasta_file)
  number_of_seqs <- length(virus_basic) #-(start+end)
  ...
  #virus_consensus<-(virus_consensus[c(start+1:(length(virus_consensus)-start))])
  ...
  #virus_DF <- virus_DF[-c(1:start), ]
  new_length <- nrow(virus_DF) 
  #virus_DF <- virus_DF[-c((new_length-end):new_length), ]
  #virus_DF<-virus_DF[c(start:(new_length-end)), ]
  #new_length <- nrow(virus_DF) 
  for(x in 1:new_length){
