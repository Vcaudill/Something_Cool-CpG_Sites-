...
plot(synonymousA$num,log10(synonymousA$MeanFreq+.000001), col = "red",xaxt="n", 
     #pch creates the shape of the plotted points
     yaxt="n",xlab="Position Number", ylab="MeanFreq",main="Synonymous A", pch=2)
...
plot(synonymousC$num,log10(synonymousC$MeanFreq+.000001), col = "magenta",xaxt="n", yaxt="n",xlab="Position Number", ylab="MeanFreq",main="Synonymous C", pch=7)
abline(h=mean(log10(synonymousC$MeanFreq+.000001)))
legend("topleft",inset=c(0,-0.45), legend=c
       ("synonymous C"),col="magenta", horiz=TRUE, lty=1:1, cex=0.8,xpd=TRUE,bty='n')

plot(nonsynonymousC$num,log10(nonsynonymousC$MeanFreq+.000001), col = "cyan",xaxt="n", yaxt="n",xlab="Position Number", ylab="MeanFreq",main="NonSynonymous C", pch = 9)
abline(h=mean(log10(nonsynonymousC$MeanFreq+.000001)))
legend("topleft",inset=c(0,-0.45), legend=c
       ("nonsynonymous C"),col="cyan", horiz=TRUE, lty=1:1, cex=0.8,xpd=TRUE,bty='n')

plot(synonymousG$num,log10(synonymousG$MeanFreq+.000001), col = "black",xaxt="n", yaxt="n",xlab="Position Number", ylab="MeanFreq",main="Synonymous G",pch=13)
abline(h=mean(log10(synonymousG$MeanFreq+.000001)))
legend("topleft",inset=c(0,-0.45), legend=c
       ("synonymous G"),col="black", horiz=TRUE, lty=1:1, cex=0.8,xpd=TRUE,bty='n')

plot(nonsynonymousT$num,log10(nonsynonymousT$MeanFreq+.000001), col = "yellow",xaxt="n", yaxt="n",xlab="Position Number", ylab="MeanFreq",main="NonSynonymous G", pch=14)
abline(h=mean(log10(nonsynonymousT$MeanFreq+.000001)))
legend("topleft",inset=c(0,-0.45), legend=c
